...
public interface ConstraintInterface {
	...
	/*Returns 'true' if the constraint applies to the given 'target'. Returns 'false' otherwise.*/
	public default boolean accepts(EObject target) {
		return appliesTo().stream().anyMatch(clss -> clss.isInstance(target));
	}
	...
	public default String getErrorMSG(EObject target) {
		return getClass().getSimpleName() + " defines no specific error message.";
	}
